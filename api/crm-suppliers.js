...
const clampLimit = (value) => Math.min(Math.max(Number.parseInt(value, 10) || 25, 1), 100);
const clampOffset = (value) => Math.max(Number.parseInt(value, 10) || 0, 0);
...
function buildSupplierQuery({ q = '', limit = 25, offset = 0, id = '' } = {}) {
  ...
  if (normalizedQuery) {
    ...
    params.set('or', `(display_name.ilike.*${escaped}*,canonical_id.ilike.*${escaped}*)`);
  }
...
async function countSuppliers(options = {}) {
  const query = new URLSearchParams({
    select: 'id', entity_type: 'eq.supplier', status: 'eq.active', limit: '1'
  }).toString();
  const result = await crmSupabaseRequest('crm_identities', { query, count: true, ...options });
...
async function listSuppliers(input = {}, options = {}) {
  const result = await crmSupabaseRequest('crm_identities', {
    query: buildSupplierQuery(input), count: true, ...options
  });
  return {
...
async function getSupplier(id, options = {}) {
  const result = await crmSupabaseRequest('crm_identities', {
    query: buildSupplierQuery({ id }), ...options
  });
...
export default async function handler(req, res) {
  ...
  if (!authorization.ok) {
    return res.status(authorization.statusCode).json({ ok: false, status: 'ACCESS_DENIED', error: authorization.error });
  }
  if (req.method !== 'GET') {
    return res.status(405).json({ ok: false, status: 'ERROR', error: 'CRM_METHOD_NOT_ALLOWED' });
  }

  const action = clean(req.query?.action || 'list').toLowerCase();
  try {
    if (action === 'count') {
      return res.status(200).json({ ok: true, status: 'READY', count: await countSuppliers() });
    }
    if (action === 'get') {
      const id = clean(req.query?.id);
      if (!id) return res.status(422).json({ ok: false, status: 'ERROR', error: 'CRM_SUPPLIER_ID_REQUIRED' });
      const supplier = await getSupplier(id);
      if (!supplier) return res.status(404).json({ ok: false, status: 'NOT_FOUND', error: 'CRM_SUPPLIER_NOT_FOUND' });
      return res.status(200).json({ ok: true, status: 'READY', supplier });
    }
    if (action === 'list' || action === 'search') {
      const result = await listSuppliers({ q: clean(req.query?.q), limit: req.query?.limit, offset: req.query?.offset });
      return res.status(200).json({ ok: true, status: 'READY', ...result });
    }
    return res.status(400).json({ ok: false, status: 'ERROR', error: 'CRM_SUPPLIER_ACTION_INVALID' });
  } catch (error) {
...
export { buildSupplierQuery, countSuppliers, getSupplier, listSuppliers, mapSupplier };
