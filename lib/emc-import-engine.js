...
function detectarMimeArchivo({ fileName = "", mimeType = "" }) {
...
function extraerPrecio(texto = "") {
  ...
  if (raw.includes(".") && !raw.includes(",")) {
    ...
    if (partes[1]?.length === 2) return Number(raw);
...
function extraerMedida(texto = "") {
  const t = String(texto || "");
  const match = t.match(/(\d+(?:[.,]\d+)?)\s*(x|×)\s*(\d+(?:[.,]\d+)?)(?:\s*(mm|cm|m|mts|yds|yd|pulg|"))?/i);
...
function procesarLineasCatalogo({ texto = "", proveedor = null }) {
...
async function analizarImportacionEMC(input = {}) {
  const body = input.body || input;
  const proveedor = normalizarProveedor(body.proveedor, body);

  const archivos = Array.isArray(body.archivos_locales)
    ? body.archivos_locales
  ...
  if (!textoUnificado && !multimedia.length) {
    return {
      ...
      siguiente_paso: "Subir PDF con texto seleccionable, Excel, CSV o TXT.",
    };
  }
...
module.exports = {
  ...
  extraerCodigo,
  detectarMarca,
  procesarLineasCatalogo,
  analizarImportacionEMC,
  importarEMC: analizarImportacionEMC,
  importarCatalogoEMC: analizarImportacionEMC,
  procesarImportacionEMC: analizarImportacionEMC,
  processEMCImport: analizarImportacionEMC,
};