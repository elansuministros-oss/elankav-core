...
function validateWhatsapp(value) {
  ...
  return whatsapp.length >= 8 ? whatsapp : '';
}
...
export function validateSupplierInput(input = {}) {
  ...
  const categories = Array.isArray(input.categories)
    ...
    : [];
  const whatsapp = validateWhatsapp(input.whatsapp || input.phone);
  ...
  if (!whatsapp) {
    return { ok: false, error: 'CRM_SUPPLIER_WHATSAPP_REQUIRED' };
  }
...
export function validateClientInput(input = {}) {
  ...
  if (!whatsapp) {
    return { ok: false, error: 'CRM_CLIENT_WHATSAPP_REQUIRED' };
  }
...
export function validateContactInput(input = {}) {
  ...
  const whatsapp = validateWhatsapp(input.whatsapp);

  if (!identityId) return { ok: false, error: 'CRM_CONTACT_IDENTITY_REQUIRED' };
  if (!whatsapp) return { ok: false, error: 'CRM_CONTACT_WHATSAPP_REQUIRED' };
