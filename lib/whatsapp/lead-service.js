...
function hasExtendedLeadColumns(lead = {}) {
  return Object.keys(lead).some((key) => !BASE_LEAD_COLUMNS.has(key));
}
...
function buildLead(normalized = {}, salesResult = {}) {
  if (salesResult?.leadFields) {
    return salesResult.leadFields;
  }

  if (salesResult?.analysis || salesResult?.responseText) {
    return buildLeadFieldsFromSalesResult(normalized, salesResult);
  }
...
async function saveLeadPayload({ existingLead, lead }) {
  const result = existingLead ? await updateLead(existingLead, lead) : await insertLead(lead);

  if (result.ok || !hasExtendedLeadColumns(lead)) return { result, lead, usedFallback: false };

  const baseLead = filterBaseLeadColumns(lead);
  const fallbackResult = existingLead ? await updateLead(existingLead, baseLead) : await insertLead(baseLead);

  return {
    result: fallbackResult.ok ? fallbackResult : result,
    lead: fallbackResult.ok ? baseLead : lead,
...
export async function saveLeadFromWahaEvent(normalized = {}, salesResult = {}) {
  ...
  const saved = await saveLeadPayload({ existingLead, lead });
  ...
  return {
    ...
    originalError: saved.originalError || null,
    data: result.data,
    lead: saved.lead,
  };
}
