...
export function normalizeWahaEvent(event = {}) {
  ...
  const participant = firstDefined(
    message.author,
    message.participant,
  ...
  const from = firstDefined(message.from, payload.from, chatId);

  const normalized = {
    source: 'WAHA',
    event: firstDefined(event.event, event.type, payload.event, payload.type, 'unknown'),
    eventId: firstDefined(event.id, event.eventId, event.event_id, payload.eventId, payload.event_id, ''),
    ...
    participant,
    lid,
    customerPhone: normalizePhone(firstDefined(message.sender?.id, message.from, payload.from, chatId, participant, lid)),
    fromMe,
