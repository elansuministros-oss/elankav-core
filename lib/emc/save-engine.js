...
async function insertWithFallback({ supabase, rows }) {
  ...
  for (const row of rows) {
    ...
    if (row.codigo_catalogo) {
      query = supabase
        .from("elankav_catalogo_proveedor_items")
        .select("id")
        .eq("proveedor_id", row.proveedor_id)
        .eq("codigo_catalogo", row.codigo_catalogo)
        .limit(1);
    }

    const { data: existentes, error: errorBuscar } = await query;

    if (errorBuscar) {
      return { ok: false, error: errorBuscar.message };
    }
