...
export function parseCatalogText({ text = "", pagina = 1, context = {} } = {}) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (/^(NE[ÓO]N FLEX|CINTAS LED|TRANSFORMADORES|M[ÓO]DULOS LED|MATERIALES|ACCESORIOS|TECNOLOG[ÍI]A)/i.test(upper)) {
      currentCategory = titleCase(line);
      continue;
    }

    if (!/(C\$|C\s*\$|USD|US\$|\$\s*\d)/i.test(line)) continue;

    const money = parseMoney(line);
    if (!money.precio) continue;

    const previous = [lines[i - 3], lines[i - 2], lines[i - 1]]
      .filter(Boolean)
      .filter((candidate) => !/(C\$|USD|US\$|\$\s*\d)/i.test(candidate))
      .filter((candidate) => candidate.length >= 3)
      .join(" ");

    const cleanedName = cleanText(
      line
        .replace(/(?:C\$|C\s*\$|USD|US\$|\$)?\s*[0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]+)?/gi, "")
        .replace(/\s+/g, " ")
    );

    const nombre = cleanedName.length >= 4 ? cleanedName : previous || `Producto página ${pagina}`;

    items.push(normalizeProductItem({
      nombre,
      descripcion: `${previous} ${line}`.trim(),
      categoria: currentCategory === "General" ? inferCategory(`${previous} ${line}`) : currentCategory,
      precio_numero: money.precio,
      moneda: money.moneda,
      linea_original: line,
      pagina,
    }, { pagina, categoria: currentCategory, proveedor_nombre: context.proveedor_nombre }));
  }
