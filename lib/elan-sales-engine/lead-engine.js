...
export function buildSalesLeadSummary(analysis = {}) {
  ...
  const pricing = analysis.pricing?.canMentionPrice ? `Precio publico: ${analysis.pricing.price?.text}` : "Precio: validar ECE/AI-23";
  const marketing =
  ...
  return compactText(`${product} | ${pricing} | ${materials} | ${imc} | ${followUp} | ${marketing}`, 260);
}
...
export function buildLeadFieldsFromSalesResult(normalized = {}, salesResult = {}) {
  ...
  return {
    origen: "WAHA",
    nombre: analysis.customerProfile?.name || normalized.from || normalized.chatId || "Cliente WhatsApp",
    whatsapp: analysis.customerProfile?.phone || stripWhatsAppSuffix(normalized.chatId || normalized.from || ""),
    mensaje: message,
    ...
    creado_por: analysis.orchestrator || "ELAN AI Orchestrator V1",
    listo_para_crm: true,
    procesado: false,
    source: "WAHA",
    ...
    customer_key: analysis.customerProfile?.customerKey || null,
    customer_profile: analysis.customerProfile ? JSON.stringify(analysis.customerProfile).slice(0, 4000) : null,
    memoria_comercial: analysis.memory ? JSON.stringify(analysis.memory).slice(0, 4000) : null,
    catalogo_publico: analysis.catalog?.bestMatch ? JSON.stringify(analysis.catalog.bestMatch).slice(0, 2000) : null,
    precio_publicado: analysis.pricing?.canMentionPrice ? analysis.pricing.price?.text || null : null,
    generador_respuesta: analysis.generator || null,
  };
}
