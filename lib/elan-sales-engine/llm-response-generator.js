...
function stepQuestionIsValid(step = "", question = "", memory = {}) {
  ...
  const expectsPlacement = !context.campaignContext && (step === "ask_placement" || (step === "share_public_price" && memory.measure && !memory.placement));
...
function nextQuestionForStep(step = "", memory = {}, product = {}, campaignContext = null) {
  if (campaignContext?.id === "rotuloLuminosoGym01") {
    if (!memory.businessType) return "¿Qué tipo de negocio tienes?";
    if (!memory.visitLocation) return "¿En qué ciudad será instalado el rótulo?";
    if (!memory.logoStatus) return "¿Ya tienes tu logo listo o también necesitas ayuda con el diseño?";
    if (!memory.measure) return "¿Te interesa el tamaño estándar de 1.10 metros (desde USD 350) o necesitas uno más grande?";
  }
...
function buildFallbackResponse(context = {}) {
  ...
  const question = nextQuestionForStep(step, memory, product, context.campaignContext);
  const priceLine = buildCatalogPriceLine(pricing);

  // New rule for specific keywords
  const clientMessage = context.normalized?.body || "";
  const keywords = ["sugerencia", "dónde conviene", "donde conviene", "tamaño", "foto", "este es mi negocio"];
  const containsKeywords = keywords.some(keyword => clientMessage.includes(keyword));

  if (containsKeywords) {
    return compactResponse([
      "Gracias por la información y la foto. Para ese tipo de fachada, lo recomendable es revisar proporción, visibilidad desde la calle y punto de instalación antes de definir el tamaño. Podemos prepararte una propuesta adecuada para tu negocio. ¿En qué ciudad será instalado?"
    ].join("\n"));
  }
...
// Si campaignContext existe:
// - Utiliza ese contexto como prioridad.
// - Sigue sus reglas comerciales.
// - Usa exactamente la comunicación de precio definida.
// - No pidas nombre, teléfono ni correo al inicio.
// - Haz solamente la siguiente pregunta necesaria del flujo.
  ].join("\n\n");
}
